...
// Fog
const fog = new THREE.Fog(clearColor, 1, 15);
scene.fog = fog;
/**
 * Textures
 */
...
for (const bushOptions of BushPositions) {
  const bush = new THREE.Mesh(bushGeometry, bushMaterial);

  bush.scale.set(...bushOptions.scale);
  bush.position.set(...bushOptions.position);
...
const tick = () => {
  ...
  ghost3.position.x = Math.cos(ghost3Angle) * (8 + Math.sin(elapsedTime * 0.5));
  ghost3.position.z = Math.sin(ghost3Angle) * (8 + Math.sin(elapsedTime * 0.5));
  ghost3.position.y = Math.sin(ghost3Angle * 3) * Math.sin(ghost3Angle * 4.5);
